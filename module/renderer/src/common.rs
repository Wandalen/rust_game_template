...
pub fn window_redraw_handle(c : &mut Context)
{
  ...
  c.queue
    .write_buffer(&c.time_buffer, 0, unsafe { any_as_u8_slice(&c.time_uniform_data) });
      ...
      Some(c) =>
      {
        match event
        {
          ...
          winit::event::Event::WindowEvent {
            ...
            ..
          } => window_close_request_handle(control_flow),
          winit::event::Event::WindowEvent {
            event:
